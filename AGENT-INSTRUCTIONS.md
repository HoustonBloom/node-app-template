...
### CLUSTERS and ORDER
Keep the `anchor` entry. Replace the rest with 2 to 7 groups that answer the question, each with a label, a distinct color, AND a distinct `shape` (circle, square, triangle, diamond, hexagon, pentagon, star). Seven shapes ship, so seven groups is the ceiling; past that two groups would have to share one and the map stops being readable without color. The shape draws on the map and in the filter chips, so a color-blind reader can still tell groups apart. List every non-anchor key in `ORDER`; that sets the sectors around the hub.
...
**A group with a long tail can carry sub-groups.** Give the cluster a `subs` map (`subs:{ key:{label:"..."} }`) and every node in it a matching `subgroup:"key"`. While that group is the active filter its sub-groups appear as a second chip row. Sub-groups are optional and no group in the shipped map uses them, since splitting the long tail into real groups read better than nesting it; the capability is there for a map whose tail genuinely belongs together under one name. Do not confuse `subgroup` with `sub`, which is the node's provenance line.

**Group labels name what the reader gets, not what you did.** "Start here", "Best practices", "Examples to try", "Worth reading" tell someone where to go. "Miscellaneous", "Other", "Additional resources" do not, which is why the shipped map has none of them: what began as one twelve-node Additional resources became Apps to connect, Next steps, Worth reading, and More tools, three nodes each. Prefer several honest names over one bucket.

**Count the cost of a group on a phone.** Each group is a chip, and at 375px the shipped map's eight chips wrap to four rows, pushing the canvas down the page. That is survivable because tapping a chip scrolls the map into view, but it is the reason to stop at the number of groups the content actually needs rather than the seven the shapes allow.
